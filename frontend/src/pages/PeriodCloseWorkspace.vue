<template>
  <JournalLikeShell
    title="Period Close"
    subtitle="Manage accounting periods and period closing vouchers."
    :companies="companies"
    :company="company"
    :from-date="fromDate"
    :to-date="toDate"
    :kpis="kpis"
    :error="error"
    @update:company="onCompanyChange"
    @update:fromDate="onFromDateChange"
    @update:toDate="onToDateChange"
  >
    <template #actions>
      <button
        type="button"
        class="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md shadow-blue-600/25 transition hover:bg-blue-700"
        @click="router.push({ name: 'period-close-new' })"
      >
        + New Period Closing Voucher
      </button>
      <button
        type="button"
        class="inline-flex items-center gap-2 rounded-xl border-2 border-violet-500 bg-white px-4 py-2.5 text-sm font-semibold text-violet-700 transition hover:bg-violet-50"
        @click="router.push({ name: 'accounting-periods' })"
      >
        Accounting Periods
      </button>
    </template>

    <template #filters>
      <button
        type="button"
        class="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-100"
        @click="load"
      >
        Refresh
      </button>
    </template>

    <template #table>
      <table class="w-full min-w-[980px] text-left text-xs">
        <thead class="border-b border-slate-200 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
          <tr>
            <th class="px-3 py-3">Voucher</th>
            <th class="px-3 py-3">Posting Date</th>
            <th class="px-3 py-3">Fiscal Year</th>
            <th class="px-3 py-3">Period Start</th>
            <th class="px-3 py-3">Period End</th>
            <th class="px-3 py-3">Status</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="row in vouchers" :key="row.name" class="border-b border-slate-50 hover:bg-slate-50/80">
            <td class="px-3 py-2.5 font-mono text-blue-600">{{ row.name }}</td>
            <td class="px-3 py-2.5 text-slate-700">{{ row.posting_date }}</td>
            <td class="px-3 py-2.5 text-slate-700">{{ row.fiscal_year }}</td>
            <td class="px-3 py-2.5 text-slate-700">{{ row.period_start_date }}</td>
            <td class="px-3 py-2.5 text-slate-700">{{ row.period_end_date }}</td>
            <td class="px-3 py-2.5">
              <span class="rounded-full px-2.5 py-0.5 text-[10px] font-semibold" :class="row.docstatus === 1 ? 'bg-emerald-50 text-emerald-700' : 'bg-blue-50 text-blue-700'">
                {{ row.docstatus === 1 ? 'Submitted' : 'Draft' }}
              </span>
            </td>
          </tr>
          <tr v-if="!vouchers.length">
            <td colspan="6" class="px-3 py-12 text-center text-sm text-slate-400">No period closing vouchers in this range.</td>
          </tr>
        </tbody>
      </table>
    </template>

    <template #footer>
      <span>{{ vouchers.length }} period close vouchers</span>
      <span class="font-medium text-slate-700">{{ periods.length }} accounting periods loaded</span>
    </template>

    <template #rightRail>
      <h3 class="text-sm font-bold text-slate-900">Accounting Period Locks</h3>
      <ul class="mt-3 space-y-2 text-xs text-slate-600">
        <li v-for="p in periods.slice(0, 8)" :key="p.name">
          {{ p.name }} ({{ p.start_date }} → {{ p.end_date }}) - {{ p.closed ? 'Closed' : 'Open' }}
        </li>
      </ul>
    </template>
  </JournalLikeShell>
</template>

<script setup>
import { computed, onMounted, ref, watch } from 'vue'
import { useRouter } from 'vue-router'
import { frappeRequest } from 'frappe-ui'
import JournalLikeShell from '@/components/workbench/JournalLikeShell.vue'
import { callWorkbench } from '@/utils/workbenchApi'
import { currentMonthBounds } from '@/utils/dateRange'

const companies = ref([])
const router = useRouter()
const company = ref('')
const fromDate = ref('')
const toDate = ref('')
const vouchers = ref([])
const periods = ref([])
const data = ref(null)
const error = ref('')

const kpis = computed(() => [
  { id: 'pcv', label: 'Period Close Vouchers', icon: '◷', value: Number(data.value?.kpis?.period_close_vouchers || 0), subtitle: 'In selected range' },
  { id: 'submitted', label: 'Submitted', icon: '✓', value: Number(data.value?.kpis?.submitted_vouchers || 0), subtitle: 'Locked closings' },
  { id: 'closed', label: 'Closed Periods', icon: '🔒', value: Number(data.value?.kpis?.closed_periods || 0), subtitle: 'Accounting period locks' },
  { id: 'company', label: 'Company', icon: '🏢', value: company.value || '—', subtitle: 'Current company' },
  { id: 'range', label: 'Date Scope', icon: '📅', value: `${fromDate.value || ''} → ${toDate.value || ''}`, subtitle: 'Posting date window' },
])

async function loadCompanies() {
  companies.value = (await frappeRequest({
    url: '/api/method/frappe.client.get_list',
    method: 'POST',
    params: { doctype: 'Company', fields: ['name'], limit_page_length: 100 },
  })) || []
  if (!company.value && companies.value.length) company.value = companies.value[0].name
}

async function load() {
  if (!company.value) return
  error.value = ''
  try {
    const res = await callWorkbench('period_close_workspace', {
      company: company.value,
      from_date: fromDate.value,
      to_date: toDate.value,
    })
    data.value = res
    vouchers.value = res.vouchers || []
    periods.value = res.accounting_periods || []
  } catch (e) {
    error.value = e.message || String(e)
  }
}

function onCompanyChange(v) { company.value = v }
function onFromDateChange(v) { fromDate.value = v }
function onToDateChange(v) { toDate.value = v }

watch([company, fromDate, toDate], load)

onMounted(async () => {
  const range = currentMonthBounds()
  fromDate.value = range.from
  toDate.value = range.to
  await loadCompanies()
  await load()
})
</script>
